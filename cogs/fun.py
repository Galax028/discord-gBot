...
class FunCog(commands.Cog):
    ...
    async def _8ball(self, ctx, *, question):
        async with aiofiles.open(f"{self.bot.path}/data/8ball.json", "r") as f:
    ...
    async def tosscoin(self, ctx, *, guess):
        ...
        embed.add_field(
            name=f"Your guess: {guess}",
            value=f"Result: {random.choice(responses)}"
        )
        await ctx.send(embed=embed)
    ...
    async def kill(self, ctx, member: discord.Member):
        ...
        rng = list(range(1, 99))
        responses = [
            f"*gunshots* {member.mention} dead, are you satisfied?",
    ...
    async def createembed(self, ctx):
        ...
        embed_color[user_id] = await self.bot.wait_for(
            "message",
            timeout=30,
        ...
        try:
            embed_init[user_id] = await self.bot.wait_for(
                "message",
                timeout=180,
                check=lambda message: message.author == ctx.author
            )
            try:
                ...
                embed[user_id] = discord.Embed(
                    title=embed_title.get(user_id),
                ...
                embed[user_id] = discord.Embed(
                    title=embed_init[user_id].content,
                    colour=embed_color.get(user_id)
                )
            ...
            number_of_fields[user_id] = await self.bot.wait_for(
                "message",
                timeout=30,
                check=lambda message: message.author == ctx.author
            )
            ...
            is_inline[user_id] = await self.bot.wait_for(
                "message",
                timeout=15,
                check=lambda message: message.author == ctx.author
            )
            ...
            for i in range(number_of_fields.get(user_id)):
                ...
                embed[user_id].add_field(
                    name=heading.get(user_id),
            ...
            embed_author[user_id] = await self.bot.wait_for(
                "message",
                timeout=60,
                check=lambda message: message.author == ctx.author
            )
            if embed_author[user_id].content.lower == "skip":
                ...
                embed[user_id].set_author(
                    name=embed_author_content,
            ...
            embed_footer[user_id] = await self.bot.wait_for(
                "message",
                timeout=60,
                check=lambda message: message.author == ctx.author
            )
            if embed_footer[user_id].content.lower == "skip":
                ...
                embed[user_id].set_footer(
                    text=embed_footer_content,
            ...
            embed_thumbnail[user_id] = await self.bot.wait_for(
                "message",
                timeout=30,
                check=lambda message: message.author == ctx.author
            )
            ...
            embed_image[user_id] = await self.bot.wait_for(
                "message",
                timeout=30,
                check=lambda message: message.author == ctx.author
            )
